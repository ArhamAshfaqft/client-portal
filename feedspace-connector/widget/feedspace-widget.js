...
`;
  ...
  function lightbox(atts, start) {
    ...
    const render = () => {
      ...
      if (a.type === "image") {
        ...
        c = `<div style="display:flex;flex-direction:column;align-items:center;gap:24px;padding:20px;"><div style="width:48px;height:48px;color:rgba(255,255,255,0.3);">${SVG_ICONS.music}</div><div style="font-size:14px;color:rgba(255,255,255,0.5);">${escHtml2(a.name || "Audio")}</div><audio controls style="width:420px;max-width:85vw;" src="${escHtml2(a.url)}" preload="metadata"></audio></div>`;
      } else {
  ...
  var FeedbackListPanel = class {
    ...
    `;
      ...
      const deviceIcons = { desktop: SVG_ICONS.desktop, tablet: SVG_ICONS.tablet, mobile: SVG_ICONS.mobile };
