...
export class CommentPanel {
  ...
  private getFileIcon(name: string): string {
    ...
    if (['jpg','jpeg','png','gif','webp','svg','ico','bmp'].includes(ext)) return '🖼️';
    if (['mp4','webm','mov','avi','mkv'].includes(ext)) return '🎬';
    if (['mp3','wav','ogg','aac','flac','webm'].includes(ext)) return '🎵';
    if (['pdf'].includes(ext)) return '📄';
    if (['doc','docx'].includes(ext)) return '📝';
    if (['xls','xlsx','csv'].includes(ext)) return '📊';
    if (['zip','rar','7z','tar','gz'].includes(ext)) return '📦';
    if (['json','xml','md','txt','log'].includes(ext)) return '📃';
    return '📎';
  }

  private updateFilePreviews(container: HTMLElement): void {
    container.innerHTML = '';
