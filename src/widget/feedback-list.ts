...
function lightbox(atts: Array<{url:string;type:string;name:string;size:number}>, start: number): void {
...
export class FeedbackListPanel {
  ...
  open(annotations: Annotation[], callbacks: FeedbackListCallbacks, onClose: () => void): void {
    ...
    body.querySelectorAll('.fs-save-library').forEach(btn => {
      btn.addEventListener('click', e => {
        ...
        if (url && this.callbacks?.onSaveToLibrary) {
          ...
          this.callbacks.onSaveToLibrary(url, name).then((ok: boolean) => {
            el.innerHTML = ok
              ? '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#22c55e" stroke-width="3" style="width:10px;height:10px;"><polyline points="20 6 9 17 4 12"/></svg>'
              : '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="3" style="width:10px;height:10px;"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
